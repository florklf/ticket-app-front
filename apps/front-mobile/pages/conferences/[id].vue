  ...
  <div>
    ...
    <div class="container mx-auto">
      <div class="relative md:mb-36">
        <img v-if="conference" :src="conference.image" :alt="conference.name" class="w-full object-cover h-60 sm:h-72">
        ...
        <div class="flex flex-col gap-2 tracking-tight bg-primary text-textonprimary sm:text-4xl inline-block px-14 py-10 w-full md:w-auto md:absolute md:rounded-tr-3xl md:bottom-[-3.5em]">
          <div v-if="conference" class="flex flex-wrap gap-2">
            <Chip
              v-for="eventGenre in conference.eventGenres" :key="(eventGenre.id as number)" :pt="{
          ...
          <div class="flex flex-wrap items-center gap-2 text-3xl font-bold ">
            ...
            >
              <template #default>
                <span class="p-chip-text text-white text-xs">{{ conference.type.name }}</span>
              </template>
            </Chip>
            <template v-else>
              <div class="flex">
          ...
          <div v-if="conference" class="flex items-center gap-2 text-lg ">
            <Icon name="ic:outline-group" width="1.5rem" height="1.5rem" />
            <div class="flex flex-wrap flex-1 items-center gap-1 leading-none">
              <template v-for="(eventArtist, index) in conference.eventArtists" :key="eventArtist.id">
                <span>{{ eventArtist.artist.name }} <template v-if="index < conference.eventArtists.length - 1">/</template></span>
              </template>
            </div>
          </div>
          <template v-else>
          ...
          <div class="flex flex-col font-light text-lg gap-1">
            <div v-if="conference" class="flex items-center gap-2">
              <Icon name="ic:sharp-place" />
              <div class="flex items-center gap-1 leading-none">
                <span>{{ conference.place.name }}, {{ conference.place.city }}</span>
              </div>
            </div>
            <Skeleton v-else height="1rem" width="10rem" />
            <div v-if="conference" class="flex items-center gap-2">
              <Icon name="material-symbols:date-range" />
              <div>{{ dayjs(conference?.date).format('dddd DD MMMM YYYY, HH:mm') }}</div>
            </div>
            <Skeleton v-else height="1rem" width="10rem" />
          </div>
        </div>
          ...
          <Accordion :active-index="0">
            <AccordionTab :header="$t('event.details.description')">
              <template v-for="i in 4">
                <Skeleton v-if="!conference" :key="i" class="mb-2" />
              </template>
              <div v-if="conference" class="flex flex-col gap-3 text-base text-gray-500">
                <p>
                ...
                <p class="flex flex-col">
                  ...
                  <span>
                    <template v-for="(eventArtist, index) in conference?.eventArtists" :key="eventArtist.id">
                      <span>{{ eventArtist.artist.name }}<template v-if="index < conference.eventArtists.length - 1"> /
                      </template></span>
                    </template>
                  </span>
                </p>
            ...
            <AccordionTab :header="$t('event.details.place')">
              <iframe
                v-if="conference" class="w-full h-64" style="border:0" loading="lazy" allowfullscreen
                referrerpolicy="no-referrer-when-downgrade"
                :src="`https://www.google.com/maps/embed/v1/place?key=${config.public.googleApiKey}&q=${conference?.place?.address} ${conference?.place?.city} ${conference?.place?.zip}`"
              />
              <Skeleton v-if="!conference" height="16rem" />
              <template v-for="i in 5">
                <Skeleton v-if="!conference" :key="i" class="my-2" />
              </template>
              <p class="text-xl font-bold mt-6">
                {{ conference?.place?.name }}
              </p>
              <div class="flex flex-col">
                <span>{{ conference?.place?.address }}</span>
                <span>{{ conference?.place?.city }} {{ conference?.place?.zip }}</span>
              </div>
              <p class="mt-6">
                {{ conference?.place?.description }}
              </p>
            </AccordionTab>
          </Accordion>
        </section>
          ...
          <form>
            <div class="flex flex-col justify-between gap-4">
              ...
              <template v-for="i in 3">
                <Skeleton v-if="!conference" :key="i" height="5rem" />
              </template>
              <div
                v-for="eventSeatType in conference?.EventSeatType" :key="eventSeatType.id"
            ...
            <div class="my-4">
              <a href="#" class="group inline-flex text-sm text-gray-500 hover:text-gray-700">
                ...
                >
                  <Fieldset
                    v-for="eventSeatType in conference?.EventSeatType" :key="eventSeatType.id" class="mb-8"
...
useHead({ titleTemplate: '%s » Conferences' })
...
const { data: conference } = await useCustomFetch<Event>(`/events/${route.params.id}`)
const i18n = useI18n({ useScope: 'global' })

const items = ref([
  { label: i18n.t('navbar.conferences'), to: constantPath.CONFERENCES_PAGE },
  { label: conference.value?.name }
])
  ...
  try {
    for (const [id, quantity] of Object.entries(seatsSelection)) {
      ...
      await window.Snipcart.api.cart.items.add({
        id,
        name: conference.value?.EventSeatType.find(type => type.id === +id)?.seatType.name,
        price: conference.value?.EventSeatType.find(type => type.id === +id)?.price,
        quantity,
        url: `${config.public.apiBase}/events/seat-types/${id}`,
        customFields: [
          { name: 'event_name', value: conference.value?.name, type: 'string' },
          { name: 'event_type', value: conference.value?.type.name, type: 'string' },
          { name: 'event_date', value: dayjs(conference.value?.date).format('DD/MM/YYYY - hh[h]mm'), type: 'string' },
          { name: 'event_place', value: conference.value?.place.name, type: 'string' }
        ]
      })
